...
function goToSerieChannel() {
    router.visit(`/public/channel/${props.user.username}/serie`);
}
...
const previewTimers = ref<Map<number, ReturnType<typeof setTimeout>>>(new Map());
const PREVIEW_DURATION = 3000;

function handleMouseEnter(shortId: number) {
    const video = videoRefs.value.get(shortId);
    if (video) {
        video.play();
        ...
        previewTimers.value.set(shortId, timer);
    }
}

function handleMouseLeave(shortId: number) {
    const timer = previewTimers.value.get(shortId);
    if (timer) {
        clearTimeout(timer);
        previewTimers.value.delete(shortId);
    }
    const video = videoRefs.value.get(shortId);
    if (video) {
        video.pause();
...
onUnmounted(() => {
    previewTimers.value.forEach(timer => clearTimeout(timer));
});
</script>

<template>
    <Head :title="`${user.username} — Shorts`" />
    <div class="public-channel-wrapper">
        ...
        <div class="public-channel-content">
            ...
            <nav class="channel-tabs">
                ...
                <button class="tab-btn" @click="goToSerieChannel">Series</button>
                <button class="tab-btn active">Shorts</button>
            </nav>
            ...
            <main class="channel-content-area">
                <div class="content-grid active">
                    <div
                        ...
                        :key="`short_${short.id}`"
                        class="content-card"
                        @mouseenter="handleMouseEnter(short.id)"
                        @mouseleave="handleMouseLeave(short.id)"
                        @click="handleContentClick($event)"
                    >
                        <template v-if="authUser">
                            <Link :href="`/short?short=${short.contentable.id}`">
                                <video
                                    :ref="(el: any) => { if (el) videoRefs.set(short.id, el as HTMLVideoElement) }"
                                    :src="short.contentable.video_url"
                                    :poster="short.contentable.thumbnail_url"
                                    muted
                                    playsinline
                                    preload="metadata"
                                />
                ...
                <div v-if="shorts.length === 0" class="empty-state">
                    <p>No shorts yet.</p>
                </div>
            </main>
        </div>
        ...
        <div v-if="showLoginModal" class="modal-backdrop" @click.self="closeLoginModal">
            <div class="login-modal">
                ...
                <p class="modal-subtitle">
                    Create an account or log in to watch shorts, follow creators, and more.
                </p>
...
.content-card { position: relative; aspect-ratio: 2/3; border-radius: 12px; overflow: hidden; background-color: #000; cursor: pointer; }
