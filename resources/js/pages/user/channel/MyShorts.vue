...
import SerieChannelController from '@/actions/App/Http/Controllers/Channel/SerieChannelController';
import MyShortCard from './partials/MyShortCard.vue';
...
function goToSerieChannel() {
    router.visit(SerieChannelController());
}
    ...
    <UserDashboardLayout 
        ...
        <main class="channel-content" style="margin-bottom: 80px;">
            ...
            <div id="shortsContent" class="content-grid active">
                <MyShortCard v-for="short in shorts" :key="`my_short_${short.id}`" :short="short.contentable" :viewsCount="short.views_count ?? 0" />
            </div>
        </main>
        <br />
    </UserDashboardLayout>
