...
function scrollSuggested(direction: 'left' | 'right') {
    const container = document.getElementById('suggested-movies-row');
    if (container) {
...
<template>
    <div v-if="hasMoviesOrSeries(category)" class="slider-wrapper">
        <!-- Native Instant SVG Arrow Left -->
        <button 
            type="button" 
            class="slider-arrow prev-arrow" 
            aria-label="Scroll left"
            @click="scrollSuggested('left')"
        >
        ...
        <div id="suggested-movies-row" class="movies-row">
            <div v-for="(item, index) in mergeMoviesAndSeries(category)"
                :key="`suggested_${item.id}_card_${index + 1}`" class="movie-card">
