...
import UploadTrailerMovieController from '@/actions/App/Http/Controllers/Movie/UploadTrailerMovieController';
import DeleteTrailerMovieController from '@/actions/App/Http/Controllers/Movie/DeleteTrailerMovieController';
...
const hasVideo = computed(() => {
    // @ts-ignore
    return movie.value.trailer_video || selectedFile.value;
});

const videoName = computed(() => {
    // @ts-ignore
    return selectedFile.value?.name || (movie.value.trailer_video ? 'Current trailer' : '');
});
...
function handleDrop(event: DragEvent) {
    ...
    if (files && files.length > 0) {
        ...
        if (file.type.startsWith('video/')) {
            selectedFile.value = file;
            const input = document.getElementById('movieTrailerFile') as HTMLInputElement;
            if (input) {
    ...
    try {
        ...
        for (let chunkIndex = 0; chunkIndex < totalChunks.value; chunkIndex++) {
            ...
            await new Promise((resolve, reject) => {
                router.post(UploadTrailerMovieController.url(movie.value), formData, {
                    preserveScroll: true,
                    forceFormData: true,
                    onSuccess: (page) => {
        ...
        const fileInput = document.getElementById('movieTrailerFile') as HTMLInputElement;
        if (fileInput) {
            fileInput.value = '';
        }
    }
...
function removeVideo() {
    ...
    router.delete(DeleteTrailerMovieController.url(movie.value), {
        preserveScroll: true,
        onSuccess: (page) => {
            selectedFile.value = null;
            uploadProgress.value = 0;
...
<template>
    <Form ref="form" v-bind="UploadTrailerMovieController.form(movie)" :reset-on-success="[]"
