...
import { ref } from 'vue';
import DestroySeasonSerieController from '@/actions/App/Http/Controllers/Serie/DestroySeasonSerieController';
...
const emits = defineEmits<{
    (e: 'season-deleted'): void;
}>();

async function deleteSeason() {
    isLoading.value = true;
    try {
        router.delete(DestroySeasonSerieController.url({
            serie: props.serie,
            season: props.season.id,
        }), {
            preserveScroll: true,
        });
        emits('season-deleted');
        isOpen.value = false;
    } catch (error) {
        console.log(error);
    } finally {
        ...
        <div class="modal" :class="{ 'active': isOpen }">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title">Delete Season</h2>
                    <button class="close-modal-btn" @click="closeModal">&times;</button>
                </div>
                
                <div class="modal-body">
                    <p>Are you sure you want to delete Season {{ season.season_number }}?</p>
                    <p class="warning-text">This action cannot be undone and will delete all episodes in this season.</p>
                </div>

                <div class="modal-actions">
                    <button type="button" class="action-button cancel-btn" @click="closeModal">Cancel</button>
                    <button type="button" class="action-button delete-btn-modal" @click="deleteSeason" :disabled="isLoading">
