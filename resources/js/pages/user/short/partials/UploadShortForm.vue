...
import UploadShortController from '@/actions/App/Http/Controllers/Short/UploadShortController';
...
const hasVideo = computed(() => {
    // @ts-ignore
    return short.value.short_video || selectedFile.value;
});

const videoName = computed(() => {
    // @ts-ignore
    return selectedFile.value?.name || (short.value.short_video ? 'Current short' : '');
});
...
function handleDrop(event: DragEvent) {
    ...
    if (files && files.length > 0) {
        ...
        if (file.type.startsWith('video/')) {
            selectedFile.value = file;
            const input = document.getElementById('shortFile') as HTMLInputElement;
            if (input) {
...
const uploadChunks = async (file: File) => {
    ...
    try {
        ...
        const fileInput = document.getElementById('shortFile') as HTMLInputElement;
        if (fileInput) {
            fileInput.value = '';
        }
        isProcessingVideo.value = false;
...
function removeVideo() {
    router.delete(DeleteShortVideoController.url(short.value), {
        preserveScroll: true,
        onSuccess: (page) => {
            selectedFile.value = null;
            uploadProgress.value = 0;
...
<template>
    <Form ref="form" v-bind="UploadShortController.form(short)" :reset-on-success="[]"
        v-slot="{ errors, processing }" :options="{ preserveScroll: true }">
        <div class="form-section">
            <label class="form-label">Short Video File</label>
            <div 
                class="upload-box"
            ...
            >
                <div v-if="!hasVideo && !isUploading" class="upload-content">
                    ...
                    <p>Select your short video (vertical)</p>
                ...
                <div v-else class="video-info">
                    ...
                    <p class="video-name">
                        {{ videoName }}
                        <!-- <video :src="short.short_video_url" controls class="video-preview"></video> -->
                    </p>
            ...
            >
            <ErrorLabel :error="errors.short_video" />
        </div>
    </Form>
</template>
