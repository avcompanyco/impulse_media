    ...
    <UserDashboardLayout 
        ...
        <div class="movies-row" data-slider="slasher" style="margin-bottom: 40px;">
            <div v-for="(movie, index) in mergeInRandomOrderMoviesAndSeries(subcategory)"
                ...
                <Link v-if="movie.content.type == 'movies'" :href="ShowMovieController(movie)">
                    <img :src="movie.vertical_image_url" alt="Movie Poster">
                </Link>
                <Link v-if="movie.content.type == 'series'" :href="ShowSerieController(movie)">
                    <img :src="movie.vertical_image_url" alt="Series Poster">
                </Link>
            </div>
        </div>
        <div v-for="subcat in subcategories" :key="`subcat_${subcat.id}`" class="movies-section" style="margin-bottom: 80px;">
            ...
            <div class="movies-row" data-slider="slasher">
                <div v-for="(movie, index) in mergeInRandomOrderMoviesAndSeries(subcat)"
                    :key="`movie_${movie.id}_card_${index + 1}`" class="movie-card">
                    <Link v-if="movie.content.type == 'movies'" :href="ShowMovieController(movie)">
                        <img :src="movie.vertical_image_url" alt="Movie Poster">
                    </Link>
